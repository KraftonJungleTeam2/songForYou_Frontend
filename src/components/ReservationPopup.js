...
const ReservationPopup = ({ roomid, socket, onClose, reservedSongs, songLists, isPlaying }) => {
  ...
  const filteredSongs = (viewType === 'public' ? songLists.public : songLists.private).filter((song) => song.metadata.title.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className='overlay'>
      <div className='popup'>
              ...
              filteredSongs.map((song) => (
                <div key={song.id} className='song-item'>
                  ...
                  <div className='buttons has-addons is-right'>
                    <button className={`button is-dark`} disabled={isReserved(song.id)} onClick={(e) => !isReserved(song.id) && handleReserve(e, song)} style={{ height: '2.2rem', width: '3rem' }}>
              ...
              reservedSongs.map((reserved, index) => (
                <div className={`song-item ${index === 0 ? 'highlighted-border' : ''} ${index === 0 && isPlaying ? 'playing-overlay' : ''}`}>
                  <div
                    className='song-icon'
                    style={{
                      backgroundImage: `url(data:image/jpeg;base64,${arrayBufferToBase64(reserved.image.data)})`,
                      backgroundSize: 'cover',
                      backgroundRepeat: 'no-repeat',
                      width: '4rem',
                      height: '4rem',
                  ...
                  <div className='buttons has-addons is-right'>
                    <button className={`button is-danger`} disabled={index === 0 && isPlaying} onClick={() => handleRemove(reserved.songId)} style={{ height: '2.2rem', width: '3rem' }}>
        ...
        <div className='close-button-pop is-ghost' onClick={onClose}>
