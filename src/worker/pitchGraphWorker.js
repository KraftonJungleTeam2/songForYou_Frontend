...
self.onmessage = async (event) => {
  ...
  if (data.type === "refData") {
    if(!arraysEqual(referData, data.referenceData)){
      ...
      const temp = getFrequencyRange(Math.min(...referData.filter(num => num > 0)), Math.max(...referData));
      frequencies = temp[0];
      scales = temp[1];
...
function drawGuidelinesAndLabels(targetCtx) {
  ...
  if (Array.isArray(frequencies)) {
    frequencies.forEach((freq, index) => {
      ...
      // 가이드라인
      targetCtx.beginPath();
      targetCtx.strokeStyle = scales[index][0] === 'C' && scales[index][1]!=='#' ? 'rgba(255, 255, 255, 0.6)' : 'rgba(255,255,255,0.15)';
      ...
      if (index % 2)
        targetCtx.fillText(`${scales[index]}`, 5, y);
    });
  }
}
...
function drawPitchData(
  ...
  if (score > 0.5 && currentTimeIndex % 3 === 0) {
    if (data[currentTimeIndex])
      ...
      addStar(x0, logScale(data[currentTimeIndex-1], dimensions, frequencies), score);
  }
